document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadMatches() {
        ...
        matches = allEvents.map(e => ({
            ...
            time: localTime(e.strTimestamp), venue: e.strVenue || '', dateEvent: e.dateEvent
        }));
    }
    ...
    function renderMatches() {
        ...
        if (!matches.length) {
            matchesContainer.innerHTML = '<p style="text-align:center;color:var(--text-secondary);padding:2rem;">No hay partidos programados para hoy ni mañana.<br>¡Volvé más tarde!</p>';
            submitBtn.style.display = 'none';
            return;
        }

        // ── Player info card (if not in URL) ──
